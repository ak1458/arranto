...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return pageMetadata({
    ...
    description: t("disclaimerSubtitle"),
    path: "/legal/disclaimer",
    locale,
  });
}

export default async function Disclaimer({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("legal");
  const sections = t.raw("disclaimerSections") as Section[];
  ...
  return (
    ...
    <div className="mb-4 inline-flex border border-[#d8d9dc]/30 bg-[#d8d9dc]/10 px-3.5 py-1 text-[10px] font-mono uppercase tracking-wider text-[#d8d9dc]">
        ...
        <h1 className="font-display text-4xl text-paper">{t("disclaimerTitle")}</h1>
        <p className="mt-4 leading-relaxed text-paper/70 font-light">{t("disclaimerSubtitle")}</p>
