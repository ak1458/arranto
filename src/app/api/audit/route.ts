...
export async function POST(request: NextRequest) {
  try {
    const key = clientKey(request, "audit");
    ...
    const cleanUrl = url.replace(/[^\w\-\.\/:\?&=#%+]/gi, "").slice(0, 500);
    let target: URL;
    try {
    ...
    if (isBlockedHost(target.hostname)) {
      return NextResponse.json({ error: "That URL points to a private/internal address and cannot be audited." }, { status: 400 });
    }
